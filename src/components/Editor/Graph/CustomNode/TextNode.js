...
import { isContentImage } from "../../../utils/graph/calculateNodeSize";
...
const StyledTextNodeWrapper = styled.span`
  display: flex;
  justify-content: ${(props) => (props.$hasCollapse ? "space-between" : "center")};
  align-items: center;
  height: 100%;
  width: 100%;
`;
...
const Node = ({ node, x, y, hasCollapse = false }) => {
  const {
    id,
    text,
    width,
