...
function getTextColor({ $value, $type, $parent, theme }) {
  // per type
...
export const StyledForeignObject = styled.foreignObject`
  text-align: ${(props) => !props.$isObject && "center"};
...
export const StyledKey = styled.span`
  ...
  flex: 1;
  color: ${(props) =>
    getTextColor({ $parent: props.$parent, $type: props.$type, $value: props.$value, theme: props.theme })};
  font-size: ${(props) => props.$parent && "14px"};
  overflow: hidden;
  text-overflow: ellipsis;
  padding: ${(props) => props.$type !== "object" && "10px"};
  white-space: nowrap;
`;

export const StyledRow = styled.span`
  padding: 0 10px;
  color: ${(props) => getTextColor({ $value: props.$value, theme: props.theme })};
  display: block;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
...
export const StyledChildrenCount = styled.span`
  color: ${(props) => props.theme.NODE_COLORS.CHILD_COUNT};
  padding: 10px;
  margin-left: -15px;
`;
